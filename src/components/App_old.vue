...
const renderRadarChart = (containerRef, data, isOom = false) => {
  ...
  // 2. 将每一个类的数据映射为雷达图的一个独立系列
  const seriesData = data.map(item => ({
    name: item.className,
    value: isOom 
      ? [item.wmc, item.nof, item.dit, item.cbo]
      : [item.wmc, item.nof, item.dit, item.rfc, item.cbo]
  }));

  const option = {
    title: { 
      text: isOom ? '设计阶段各组件指标对比' : '源代码各模块指标对比', 
      left: 'center' 
    },
    tooltip: {
      trigger: 'item' // 鼠标悬停在某个系列上时显示该类的所有指标
    },
    legend: {
      bottom: 0,
      type: 'scroll', // 如果类很多，图例会自动翻页
    ...
    series: [{
      name: '类度量对比',
      type: 'radar',
      data: seriesData, // 这里传入包含多个类数据的数组
      symbol: 'circle',
      symbolSize: 6,
      emphasis: {
        lineStyle: { width: 4 }
...
.fade-enter-active,
.fade-leave-active {
  transition: opacity 0.6s ease; /* 0.6秒的淡入淡出，非常优雅 */
}
